...
public class GenerateVerificationCode(ILogger<GenerateVerificationCode> logger, IVerificationService verificationService)
{
    ...
    public async Task<string> Run(
        [ServiceBusTrigger("verification_request", Connection = "ServiceBusConnection")] ServiceBusReceivedMessage message, ServiceBusMessageActions messageActions)
    {
        try
        {
            ...
            if (verificationRequest is not null)
            {
                ...
                if (!string.IsNullOrEmpty(code))
                {
                    ...
                    if (result)
                    {
                        ...
                        {
                            var payload = _verificationService.GenerateSBEmailRequest(emailRequest);
